    ...
    using(editorExamples, (user, description) => {
        describe(`Permissions: ${description} role:`, () => {
            ...
            it('I can not Create issue without title', async () => {
                ...
                return issueCreateModal.notification
                    .assertIsError('You should fill Title and Resolution fields!', 'Fill all required fields!');
            });
            ...
            it('I can not Create issue with existing issue title', async () => {
                ...
                await issueCreateModal.save();
                return issueCreateModal.notification
                    .assertIsError('Cannot create! Issue with this title already exists in this Project!', 'Ooops! 409 code');
            });
            ...
            it('I can Create issue with only title', async () => {
                ...
                await issueCreateModal.save();
                return issueCreateModal.notification
                    .assertIsSuccess(`The issue '${onlyTitleIssue.title}' was created.`);
            });

            it('Create Issue modal is closed after creation', async () => {
                return expect(issueCreateModal.isOpened()).toBe(false, 'Create Issue modal was not closed after issue creation!');
            });

            it('I can see created issue in issues list', async () => {
                return expect(issuesList.isIssuePresent(onlyTitleIssue.title)).toBe(true, 'Created Issue does not appear in Issues List!');
            });
            ...
            it('I can fill Resolution', async () => {
                ...
                return expect(issueCreateModal.getResolution())
                    .toBe(fullIssue.resolution.name, 'Resolution was not selected');
            });

            it('I can fill Assignee', async () => {
                await issueCreateModal.setAssignee(`${fullIssue.assignee.first_name} ${fullIssue.assignee.second_name}`);
                return expect(issueCreateModal.getAssignee())
                    .toBe(`${fullIssue.assignee.first_name} ${fullIssue.assignee.second_name}`, 'Assignee was not selected');
            });

            it('I can fill External Issue', async () => {
                await issueCreateModal.setExternalIssue(fullIssue.external_url);
                return expect(issueCreateModal.getExternalIssue())
                    .toBe(fullIssue.external_url, 'External Issue was not filled');
            });

            it('I can see error message when entered overlapped Expression', async () => {
                await issueCreateModal.setExpression('.*');
                return expect(issueCreateModal.getExpressionError())
                    .toBe('Expression is overlapped!', 'Error message is not appeared');
            });

            it('I can see Issue with overlapped expression in table below', async () => {
                await expect(issueCreateModal.isOverlappedIssuesTableExist())
                    .toBe(true, 'Overlapped Issues Table does not Exist!');
                return expect(issueCreateModal.isIssueInOverlappedTable(existingIssue.title))
            ...
            it('I cannot create issue when expression is overlapped', async () => {
                await issueCreateModal.save();
                return issueCreateModal.notification
                    .assertIsError('The Regular Expression sould not be overlapped with other issues!', 'Expression is overlapped!');
            });
            ...
            it('Overlapped expressions table is hidden when expression is not overlapped', async () => {
                ...
                return expect(issueCreateModal.isOverlappedIssuesTableExist())
                    .toBe(false, 'Overlapped Issues Table Exists!');
            });

            it('I can Create issue with all field filled', async () => {
                await issueCreateModal.save();
                return issueCreateModal.notification
                    .assertIsSuccess(`The issue '${fullIssue.title}' was created.`);
            });
        });
    });
});
