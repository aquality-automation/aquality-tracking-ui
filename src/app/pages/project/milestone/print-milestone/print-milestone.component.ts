...
export class PrintMilestoneComponent extends ModalComponent implements OnInit {
  ...
  addMilestoneInfo() {
    ...
    if (this.milestone) {
      this.doc.text(75, height + 10, 'Milestone:');
    }
  }
  ...
  filterResults() {
    ...
    this.testResultsToPrint = this.testResultsToPrint.filter(x => {
      ...
      let resolution = (this.showAppIssues
        && this.showTestIssue
        && this.showOtherResolutions) || (!this.showAppIssues
        && !this.showTestIssue
        && !this.showOtherResolutions);
      if (x.issue) {
        resolution = (x.issue.resolution.color === 1 && this.showAppIssues)
